...
--------------------------------------------------
-- New commands for manual encryption/decryption --
--------------------------------------------------

-- Encrypt the current file.
-- If the file was opened decrypted by the plugin (i.e. M.encrypted_buffers exists),
-- then we use the same save function; otherwise, if the file is plain text,
-- we encrypt it permanently.
function M.vault_encrypt()
	local buf = vim.api.nvim_get_current_buf()
	local file_path = vim.api.nvim_buf_get_name(buf)
	local vault_pass = M.find_vault_pass_file(file_path)
	if not vault_pass then
		vim.notify("No vault password file found", vim.log.levels.ERROR)
		return
	end

	if M.encrypted_buffers[buf] then
		-- The buffer was opened in decrypted mode by the plugin.
		if M.save_encrypted_file(buf, true) then
			vim.cmd("edit!") -- reload to show the now-encrypted file
		end
		return
	end

	if M.is_vault_file(buf) then
		vim.notify("File is already encrypted", vim.log.levels.INFO)
		return
	end
	...
	if vim.v.shell_error == 0 then
		vim.fn.system(string.format("mv %s %s", tmp, file_path))
		vim.notify("File encrypted successfully", vim.log.levels.INFO)
		vim.cmd("edit!") -- reload the encrypted file
	else
		vim.notify("Failed to encrypt file: " .. result, vim.log.levels.ERROR)
		vim.fn.delete(tmp)
	end
...
-- If the file was opened decrypted by the plugin (via open_with_vault) then the
-- file on disk is still encrypted. This command will permanently decrypt the file.
function M.vault_decrypt()
	...
	if not M.is_vault_file(buf) and not M.encrypted_buffers[buf] then
		vim.notify("File is not encrypted", vim.log.levels.INFO)
		return
	end

	local cmd = string.format("ansible-vault decrypt --vault-password-file %s %s", vault_pass, file_path)
	local result = vim.fn.system(cmd)
	if vim.v.shell_error == 0 then
		vim.notify("File decrypted successfully", vim.log.levels.INFO)
		-- If the buffer was managed by the plugin, clear its state.
		M.encrypted_buffers[buf] = nil
		vim.api.nvim_buf_set_option(buf, "buftype", "")
		vim.cmd("edit!") -- reload the decrypted file
	else
		vim.notify("Failed to decrypt file: " .. result, vim.log.levels.ERROR)
	end
end

--------------------------------------------------
-- Setup the plugin and register commands --
--------------------------------------------------
...
function M.setup(opts)
	...
	vim.api.nvim_create_user_command("AnsibleVaultEncrypt", function()
		M.vault_encrypt()
	end, { desc = "Encrypt current file using ansible-vault" })

	vim.api.nvim_create_user_command("AnsibleVaultDecrypt", function()
		M.vault_decrypt()
	end, { desc = "Decrypt current file using ansible-vault" })
end
